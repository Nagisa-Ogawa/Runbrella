...
public class SceneController : MonoBehaviour
{
    ...
    // 各プレイヤーのプレイヤーコンポーネント
    public Dictionary<PLAYER_NO, Player> players = new Dictionary<PLAYER_NO, Player>();
    // ゲームがスタートしているかどうか
    public bool isStart;
    // 誰かがゴールしているか
    public bool isEnd;
        ...
        { 
            ...
            if (GamePad.GetButtonDown(GamePad.Button.A, GamePad.Index.Any) || Input.GetKeyDown(KeyCode.Return))
            {
                ...
                for (int i = 0; i < GameManager.Instance.playerNumber; i++)
                {
                    ...
                    playerinfo.playerNo = goalRunkOrder[i].GetComponent<Player>().playerNO;
                    playerinfo.charType = goalRunkOrder[i].GetComponent<Player>().charType;
