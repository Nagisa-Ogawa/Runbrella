...
public class GoalCoinUI : MonoBehaviour
{
    // 高さ
    ...
    // スタート時の移動スピード(秒)
    public float startMoveSpeedTime = 0;
    // スタート時の拡大率
    public float startCoinSizeMax = 0;
    // 終了時の移動スピード(秒)
    public float endMoveSpeedTime = 0;
    // 終了時の拡大率
    ...
    private void ReadTextParameter()
    {
        ...
        try
        {
            ...
            //startMoveSpeedTime = GoalCoinDictionary["ゴールコインが旗から出て真ん中に行くまでの時間(フレーム数)"];
            startCoinSizeMax = GoalCoinDictionary["ゴールコインの最大サイズ"];
            //endMoveSpeedTime = GoalCoinDictionary["ゴールコインが真ん中からUIに行くまでの時間(フレーム数)"];
            endCoinSizeMini = GoalCoinDictionary["ゴールコインの最小サイズ"];
        }
        catch
        {
            Debug.Assert(false, nameof(GoalCoinUI) + "でエラーが発生しました");
        }
    ...
    /// <param name="targetTime">移動にかける秒数</param>
    /// <param name="coinSizeMax">コインが拡大・縮小するサイズ</param>
    /// <returns></returns>
    public IEnumerator OnMove(Vector3 targetPos, float targetTime, float coinSizeMax)
    {
        // 1フレームでの移動量と拡大量を計算
        ...
        // 拡大量
        float expansion =
            (coinSizeMax - gameObject.GetComponent<RectTransform>().sizeDelta.x);
        float nowDistance = 0;
        // 目標地点まで繰り返す
        while (true)
        {
            // 1秒での移動量から1フレームあたりの移動量を計算
            float moveSpeedPer1Frame = (distance / targetTime) * Time.deltaTime;
            float expansionSpeedPer1Frame = (expansion / targetTime) * Time.deltaTime;
            // 拡大処理
            gameObject.GetComponent<RectTransform>().sizeDelta += new Vector2(1, 1) * expansionSpeedPer1Frame;
            // 移動処理
            gameObject.GetComponent<RectTransform>().localPosition += normalVec * moveSpeedPer1Frame;
            nowDistance += moveSpeedPer1Frame;
